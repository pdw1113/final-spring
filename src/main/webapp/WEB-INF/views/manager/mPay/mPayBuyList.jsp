...
<html lang="en">
   ...
   <body>
      ...
      <form action="myBuyList.do" method="POST">
      <div class="section">
         ...
         <div class="main-section">
            ...
            <div class="payment-main-bottom">
               <table class="table table-hover table-condensed font_jua botto" id="bottom-tabel">
                  ...
                  <tr id="trtr">
                     <td class="bottom-td1" id="tdtd">${p.num}</td>
                     <td class="bottom-td2" id="tdtd"><span>${p.date}</span></td>
                     <td class="bottom-td3" id="tdtd"><span>${p.email}</span><br>
                     </td>
                     <td class="bottom-td4" id="tdtd"><span>${p.master}</span></td>
                     <td class="bottom-td6" id="tdtd"><span>${p.title}</span></td>
                     <td class="bottom-td7" id="tdtd"><span id="priceRed">${p.money}원</span></td>
                  </tr>
                  </c:forEach>
                  </c:if>
                  </table>
				...
				<c:if test="${ pi.currentPage ne 1 }">
					<c:url var="before" value="myBuyList.do">
				...
				<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
					...
					<c:if test="${ p ne pi.currentPage }">
						<c:url var="WalletPagination" value="myBuyList.do">
				...
				<c:if test="${ pi.currentPage ne pi.maxPage }">
					<c:url var="after" value="myBuyList.do">
						<c:param name="currentPage" value="${ pi.currentPage + 1 }"/>
							<c:param name="preday" value="${preday}"/>
							<c:param name="postday" value="${postday}"/>
							<c:param name="selectBox" value="${selectBox}"/>
